...
Game.prototype.update = function () {
    ...
    if (this.gameWin) {
        ...
        this.storage.recordLocalTable({ time: time });
    }

    if (this.gameInfinity && this.gameOver) {
        const time = this.storage.time();
        this.storage.recordTable({ time: time, score: this.score }, true);
        this.storage.recordLocalTable({ time: time, score: this.score }, true);
    }
...
Game.prototype.move = function (data) {
    ...
    for (let i = 0; i < this.size; i++) {
        ...
        if (data == "Down" || data == "Right") {
            record = this.moveRow(row.reverse());
            record.row.reverse();
        }
        else {
            record = this.moveRow(row);
        }

        this.grid.setRow(i, record.row, data)
        moved = moved || record.moved;
    }
...
Game.prototype.openRecordTable = async function () {
    ...
    const record = await this.storage.recordTable();
    const recordI = await this.storage.recordTable(false, true);
    const local = this.storage.recordLocalTable();
    const localI = this.storage.recordLocalTable(false, true);

    this.view.openRecord([record, recordI, local, localI]);
    this.pause = true;
}
...
Game.prototype.setName =  function () {
    ...
    const checkName = async (name) => {
        ...
        let res = await this.storage.checkName(name);
